...
function loadPlayers (rawPlayers) {
...
function loadTurn ({ rawTurn, players, board }) {
...
function load (rawState) {
    const players = loadPlayers(rawState.players);
    const board = loadBoard(rawState.board);
    ...
    const turn = loadTurn({ rawTurn: rawState.turn, board, players });

    const state = State.create({
        turnCount: rawState.turnCount,
        cards: rawState.cards.slice(0),
        cardQueue: Queue.create({ items: rawState.cardQueue || rawState.cards.slice(0) }),
        players,
        playerQueue,
        board,
