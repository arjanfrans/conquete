...
class Game {
    ...
    emit (event, ...args) {
        ...
        if (playerEvents.includes(event)) {
            ...
            if (this.playerEmitters[playerId]) {
                ...
                this.state.setPreviousPlayerEvent({
                    name: eventData.name,
                    playerId: playerId,
                    data: eventData.data
                });
            ...
            this.state.setPreviousTurnEvent({
                name: event.name,
                data: eventData.data,
            });
    ...
    start () {
        ...
        const previousTurnEvent = this.state.getPreviousTurnEvent() ? Object.assign({}, this.state.getPreviousTurnEvent()) : null;
        const previousPlayerEvent = this.state.getPreviousPlayerEvent() ? Object.assign({}, this.state.getPreviousPlayerEvent()) : null;
    ...
    endTurn () {
        ...
        turn.player = this.state.getPlayerQueue().next();
    ...
    claimTerritory (territory) {
        ...
        territory.setOwner(this.turn.player);
        territory.setUnits(1);
        this.turn.player.addTerritory(territory);
    ...
    deployOneUnit (territory) {
        if (territory.getOwner() !== this.turn.player) {
    ...
    redeemCards (cards) {
        ...
        const bonus = this.state.getCardManager().getBonus(cards);

        for (const card of cards) {
            this.turn.player.removeCard(card);
            this.state.getCardManager().pushCard(card);
        }
    ...
    attack (fromTerritory, toTerritory, units) {
        ...
        if (fromTerritory.getOwner() !== this.turn.player) {
            throw createError(ERRORS.NotOwnTerritoryError, {
        ...
        if (toTerritory.getOwner() === this.turn.player) {
    ...
    _killPlayer (killer, killed) {
        this.state.getPlayerQueue().remove(killed);
    ...
    _grabCard () {
        const card = this.state.getCardManager().popCard();
