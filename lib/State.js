...
class State {
    ...
    _loadTurn (rawTurn) {
        ...
        if (rawTurn.battle) {
            const from = this.board.getTerritoryById(rawTurn.battle.from);
            const to = this.board.getTerritoryById(rawTurn.battle.to);

            battle = Battle.create({
                from,
                to,
                units: rawTurn.battle.attacker.initialUnits,
                turn: this.players.get(rawTurn.battle.turn)
            });
        ...
        for (const rawTerritory of rawTerritories) {
            for (const rawPlayer of rawPlayers) {
                if (rawPlayer.territoryIds && rawPlayer.territoryIds.includes(rawTerritory.id)) {
                    const territory = this.board.getTerritoryById(rawTerritory.id);
    ...
    toJSON () {
        const json = {
            ...
            playerQueue: this.playerQueue.items.map(player => player.getId()),
