...
const PHASE_METHODS = {
    ...
    [Game.TURN_PHASES.PLACEMENT]: ['deployUnits', 'redeemCards', 'attackPhase', 'availableUnits', 'cards'],
    [Game.TURN_PHASES.ATTACKING]: ['attack', 'fortifyPhase', 'rollDice', 'cards'],
    [Game.TURN_PHASES.FORTIFYING]: ['moveUnits', 'endTurn', 'cards']
};
            ...
            if (phaseMethods.includes(methodName)) {
                function wrappedMethod (playerId, ...args) {
                    ...
                    if (game.phase === Game.PHASES.BATTLE && game.turn.phase !== phaseName) {
                        debug('attempted: ', methodName, playerId, args);
