...
class Game {
    ...
    emit (eventName, ...args) {
        ...
        if (playerEvents.includes(eventName)) {
            ...
            if (this.playerEmitters[playerId]) {
                let playerEmitter = this.playerEmitters[playerId];
            ...
            this.gameEmitter.emit(eventName, ...args);
    ...
    rollDice (numberOfDice) {
        ...
        if (battle.attacker === battle.turn) {
            ...
            this.emit(GAME_EVENTS.ATTACK_DICE_ROLL, {
                dice: dice
            });
    ...
    _killPlayer (killer, killed) {
        ...
        this.emit(GAME_EVENTS.PLAYER_KILLED, {
            killer: killer.id,
            killed: killed.id,
